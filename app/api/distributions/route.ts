...
import { AuthError, assertSameOrigin, authErrorResponse, database, requestIdentifier, requireLeader } from "../../../lib/server/auth.ts";

export const dynamic = "force-dynamic";

type StateRow = { payload: string; revision: number };
type PublicationRow = { id: number; name: string; snapshot: string; status: string; is_current: number };
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const body = (await request.json()) as { action?: "archive" | "restore" | "delete"; distributionId?: number; reason?: string; confirmation?: string; revision?: number };
    const reason = String(body.reason || "").trim();
    if (!Number.isInteger(body.distributionId) || !body.action) return Response.json({ error: "Acción o distribución inválida.", requestId }, { status: 400 });
    const db = database();
    ...
    if (Number(body.revision) !== stateRow.revision) return Response.json({ error: "La información cambió en otra sesión. Recarga antes de continuar.", code: "REVISION_CONFLICT", requestId }, { status: 409 });

    if (body.action === "archive") {
      if (!reason) return Response.json({ error: "El motivo de archivo es obligatorio.", requestId }, { status: 400 });
      const statements = [db.prepare("UPDATE published_distributions SET status = 'archived', is_current = 0, archived_at = CURRENT_TIMESTAMP, archived_by = ?, archive_reason = ? WHERE id = ?").bind(user.displayName, reason, publication.id)];
      if (publication.is_current) {
        const state = JSON.parse(stateRow.payload) as AppState;
        state.groups = []; state.activeScheduleId = null; state.version = stateRow.revision + 1;
    ...
    if (String(body.confirmation || "").trim().toUpperCase() !== "ELIMINAR") return Response.json({ error: "Escribe ELIMINAR para confirmar.", requestId }, { status: 400 });
    ...
    const statements = [db.prepare("DELETE FROM history_events WHERE distribution_id = ?").bind(publication.id), db.prepare("DELETE FROM published_distributions WHERE id = ?").bind(publication.id)];
    if (publication.is_current) {
      const state = JSON.parse(stateRow.payload) as AppState;
      state.groups = []; state.activeScheduleId = null; state.version = stateRow.revision + 1;
      statements.push(db.prepare("UPDATE app_state SET payload = ?, revision = ?, updated_at = CURRENT_TIMESTAMP WHERE id = 1 AND revision = ?").bind(JSON.stringify(state), state.version, stateRow.revision));
