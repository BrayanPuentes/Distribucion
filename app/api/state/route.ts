...
async function ensureDatabase() {
  ...
  await db.batch([
    ...
    db.prepare("CREATE TABLE IF NOT EXISTS history_events (id INTEGER PRIMARY KEY AUTOINCREMENT, distribution_id INTEGER, effective_at TEXT NOT NULL, valid_until TEXT, shift TEXT NOT NULL, task_id INTEGER, task TEXT NOT NULL, task_description TEXT NOT NULL DEFAULT '', assignment_note TEXT NOT NULL DEFAULT '', analyst_id INTEGER, analyst TEXT NOT NULL, group_id INTEGER, group_name TEXT NOT NULL, event TEXT NOT NULL, version INTEGER NOT NULL, created_by TEXT NOT NULL, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)"),
  ...
  if (!existingColumns.has("task_id")) {
    await db.prepare("ALTER TABLE history_events ADD COLUMN task_id INTEGER").run();
  }
  if (!existingColumns.has("analyst_id")) {
    await db.prepare("ALTER TABLE history_events ADD COLUMN analyst_id INTEGER").run();
  }
  if (!existingColumns.has("group_id")) {
    await db.prepare("ALTER TABLE history_events ADD COLUMN group_id INTEGER").run();
  }
...
function normalizeTasks(value: unknown): Task[] {
  ...
  return value.map((item, index) => {
    ...
    const normalized: Task = {
      ...
      shifts: Array.isArray(task.shifts) ? task.shifts.map(String) as ShiftName[] : undefined,
      qa: Boolean(task.qa),
...
const RETIRED_TASKS = new Set(["check communications", "search for vulnerabilities", "app searches", "brand and apps manual searches", "activision"]);
const TASK_RENAMES = new Map([
  ["ads alerts", "Check teams notifications for ProactivityTA Ads"],
  ["brand searches", "Brand Abuse Protection Manual Searches"],
  ["check quick alerts", "Check Threat Alerts 3"],
  ["miarroba and dnpedia", "Phish y redirect to phish Manual Searches"],
  ["check monitoring closed tickets", "CheckTakedownAlerts"],
  ["check tickets changes notifcations", "CheckTakedownAlerts"],
  ["update following tickets files - jp br redirects", "GFC Tracking"],
  ["new takedowns & inprogress (night)", "New Takedowns & Inprogress"],
]);

function migrateTaskCatalog(tasks: Task[]) {
  const migrated: Task[] = [];
  const seen = new Set<string>();
  for (const item of tasks) {
    const normalized = item.name.trim().toLocaleLowerCase("es");
    if (RETIRED_TASKS.has(normalized)) continue;
    const next = TASK_RENAMES.has(normalized) ? { ...item, name: TASK_RENAMES.get(normalized)! } : item;
    const key = next.name.trim().toLocaleLowerCase("es");
    if (seen.has(key)) continue;
    seen.add(key);
    migrated.push(next);
  }
  let nextId = Math.max(0, ...migrated.map((item) => item.id)) + 1;
  for (const desired of initialTasks) {
    const existing = migrated.find((item) => item.name.trim().toLocaleLowerCase("es") === desired.name.toLocaleLowerCase("es"));
    if (existing) Object.assign(existing, { category: desired.category, minAnalysts: desired.minAnalysts, family: desired.family, shifts: desired.shifts, qa: desired.qa, exclusive: desired.exclusive });
    else migrated.push({ ...desired, id: nextId++ });
  }
  return migrated;
}

function normalizeTemplates(value: unknown, tasks: Task[]): DistributionTemplate[] {
  const usingInitial = !(Array.isArray(value) && value.length);
  const source = usingInitial ? initialTemplates : value as DistributionTemplate[];
  const desiredIdToCurrentId = new Map(initialTasks.map((item) => [item.id, tasks.find((candidate) => candidate.name === item.name)?.id]));
  const validIds = new Set(tasks.map((item) => item.id));
  return source.map((item) => item as Partial<DistributionTemplate>).filter((item) => item.shift && Number.isInteger(item.analystCount) && Array.isArray(item.groups)).map((item) => ({
    id: String(item.id || `${item.shift}-${item.analystCount}`),
    name: String(item.name || `${item.shift} · ${item.analystCount} analistas`),
    shift: String(item.shift) as ShiftName,
    analystCount: Number(item.analystCount),
    active: item.active !== false,
    groups: (item.groups || []).map((templateGroup, index) => ({
      id: Number(templateGroup.id) || index + 1,
      name: String(templateGroup.name || `Grupo ${index + 1}`),
      taskIds: (templateGroup.taskIds || []).map(Number).map((id) => usingInitial ? desiredIdToCurrentId.get(id) : validIds.has(id) ? id : undefined).filter((id): id is number => Number.isInteger(id)),
    })),
  }));
}
...
function normalizeState(value: unknown): AppState {
  ...
  const tasks = migrateTaskCatalog(normalizeTasks(source.tasks));
  return {
    ...
    templates: normalizeTemplates(source.templates, tasks),
...
function historyStatements(
  ...
  return groups.flatMap((group) => {
    ...
    return group.taskIds.map((taskId) => {
      ...
      return db
        .prepare("INSERT INTO history_events (distribution_id, effective_at, valid_until, shift, task_id, task, task_description, assignment_note, analyst_id, analyst, group_id, group_name, event, version, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
        .bind(
          distributionId,
          effectiveAt,
          validUntil,
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [history, published, audit, logs] = await Promise.all([
      db.prepare("SELECT id, distribution_id, effective_at, valid_until, shift, task_id, task, task_description, assignment_note, analyst_id, analyst, group_id, group_name, event, version, created_by FROM history_events ORDER BY effective_at DESC, id DESC LIMIT 5000").all(),
      db.prepare("SELECT id, schedule_id, name, effective_at, valid_until, shift, snapshot, status, is_current, archived_at, archived_by, archive_reason, created_by, created_at FROM published_distributions ORDER BY effective_at DESC, id DESC").all(),
      user.role === "leader"
